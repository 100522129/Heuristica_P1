...
s.t. RDisponibilidad {i in AUTOBUS, t in TALLER, s in FRANJA}:
    x[i,t,s] <= O[s,t];

# Capacidad: cada (taller, franja) solo puede atender a un autobús
s.t. Un_autobus_Maximo {t in TALLER, s in FRANJA}:
    sum {i in AUTOBUS} x[i,t,s] <= 1;

# Cada autobús se asigna exactamente una vez (a alguna franja de algún taller)
s.t. Siempre_Asignado {i in AUTOBUS}:
    sum {t in TALLER, s in FRANJA} x[i,t,s] = 1;

# Definición de z[i,s]: se activa si el autobús i usa la franja s en cualquier taller
s.t. Autobus_Franja {i in AUTOBUS, s in FRANJA}:
    z[i,s] = sum {t in TALLER} x[i,t,s];

# Definición de y[i,j,s]: se activa si ambos autobuses i y j están en la misma franja s
s.t. RY1 {i in AUTOBUS, j in AUTOBUS, s in FRANJA}:
    y[i,j,s] <= z[i,s];

s.t. RY2 {i in AUTOBUS, j in AUTOBUS, s in FRANJA}:
    y[i,j,s] <= z[j,s];

s.t. RY3 {i in AUTOBUS, j in AUTOBUS, s in FRANJA}:
    y[i,j,s] >= z[i,s] + z[j,s] - 1;
...
minimize CosteTotal:
    0.5 * sum {i in AUTOBUS, j in AUTOBUS, s in FRANJA} C[i,j] * y[i,j,s];
